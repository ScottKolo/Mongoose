...
\title{Mongoose User Guide, Version 1.0.0}
...
./bin/Demo
********************************************************************************
Mongoose Graph Partitioning Library, Version 1.0.0
...
is represented by the following components:
    ...
    \item \texttt{Int i [nz]}: the adjacency lists held in a single array.  The adjacency
        ...
        TODO (check this:)? The vertices in each adjacency list must appear in ascending order.
...
\textbf{\texttt{Graph *readGraph(const std::string \&filename);}}\\
\textbf{\texttt{Graph *readGraph(const char *filename);}}\\
...
\textbf{\texttt{Mongoose::readGraph(``../Matrix/jagmesh7.mtx");}}
...
\vspace{6pt}
\item \textbf{\texttt{$\sim$Options();}} is the destructor for the \texttt{Options} struct. If the user creates an \texttt{Options} struct using \texttt{Options::Create}, the user is also responsible for destructing it.
...
Also note that Mongoose will NOT free pointers passed to it, and that all pointers are shallow copies (i.e. Mongoose does not make a copy of any data passed into it). Freeing memory referenced by Mongoose prior to Mongoose completing will result in a segmentation fault.
...
\texttt{coarsen} is used to coarsen an adjacency matrix (\texttt{G}) one level (one round of matching). An optional options struct (\texttt{Opts}) can be specified, as well as vertex weights (\texttt{A}).
\\
...
\texttt{edgecut} computes an edge cut of the graph \texttt{G} with options \texttt{Opts} and vertex weights \texttt{A}, such that \texttt{A(i)} = $\text{weight}(v_i)$. The returned array, \texttt{partition}, is a $1 \times n$ binary array such that
\[
\texttt{partition(i)} = 
  \begin{cases} 
...
\texttt{safe\_coarsen} attempts to coarsen a graph \texttt{G} with options \texttt{Opts} and vertex weights \texttt{A}. Prior to coarsening, \texttt{safe\_coarsen} first calls \texttt{sanitize(G)} to ensure that the graph is able to be coarsened.
\\
\item \textbf{\texttt{function partition = safe\_edgecut(G, \textit{Opts}, \textit{A})}}\\
\texttt{safe\_edgecut} attempts to compute and edge cut for a graph \texttt{G} with options \texttt{Opts} and vertex weights \texttt{A}. Note that both \texttt{Opts} and \texttt{A} are optional arguments. \texttt{safe\_edgecut} first calls \texttt{sanitize(G)} to ensure that the graph is formatted correctly.
\\ 
...
\item \texttt{HEMSRdeg}, heavy edge matching with stall-reducing matching subject to a degree threshold. Same as \texttt{HEMSR}, but the stall-reducing step is only attempted on unmatched vertices whose degree is above a threshold, described by $\texttt{Options::highDegreeThreshold}*\text{(average degree of graph)}$. \texttt{highDegreeThreshold} is set to $2.0$ by default, meaning only unmatched vertices with degree greater than or equal to two times the average degree of the graph are considered for stall-reducing matching.
\end{itemize}
...
\vskip 1\baselineskip
\begin{tabular}{|l|l|} \hline
Name & \texttt{highDegreeThreshold} \\ \hline
...
\[
\text{degree}(v) \geq \lfloor(\texttt{highDegreeThreshold}) \cdot \left(\frac{nz}{n}\right)\rfloor
\]
