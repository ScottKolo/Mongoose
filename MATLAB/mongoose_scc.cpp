...
)
{
    ...
    n = A->n ;
    Ap2 = (csi *) SuiteSparse_malloc (n+1, sizeof (csi)) ;
    for (j = 0 ; j <= n ; j++) Ap2 [j] = A->p [j] ;
    A->p = Ap2 ;
    D = cs_scc (A) ;                                    /* find conn. comp. */
    pargout [0] = gp_mex_put_int (D->p, n, 1, 0) ;              /* return p */
    pargout [1] = gp_mex_put_int (D->r, D->nb+1, 1, 0) ;        /* return r */
    cs_dfree (D) ;
    SuiteSparse_free (Ap2) ;     /* free the copy of A->p */
}
