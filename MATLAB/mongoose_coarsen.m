function [G_coarse, A_coarse, map] = mongoose_coarsen (G, O, A)
...
if (nargin == 1)
    [G_coarse, A_coarse, map] = mongoose_coarsen_mex(G_safe);
elseif (nargin == 2)
    [G_coarse, A_coarse, map] = mongoose_coarsen_mex(G_safe, O);
else
    [G_coarse, A_coarse, map] = mongoose_coarsen_mex(G_safe, O, A);
end